...
bundle_path="/home/olpc/Activities/Blender.activity"
...
class TerminalActivity(sugar.activity.activity.Activity):
    ...
    def __init__(self, handle):
        ...
        self.set_title(_('Blender Terminal Activity'))
	
        os.environ['LD_LIBRARY_PATH'] = "%s:%s" % (os.path.join(bundle_path, 'lib'), os.environ.get('LD_LIBRARY_PATH', ''))
        os.environ['PATH'] = "%s:%s" % (os.path.join(bundle_path, 'bin'), os.environ.get('PATH', ''))
    ...
    def _become_root_cb(self, button):
        vt = self.notebook.get_nth_page(self.notebook.get_current_page()).vt
        vt.feed('\r\n')
        vt.fork_command("/usr/bin/aplay ./house_lo.wav")
    ...
    def _configure_vt(self, vt):
        ...
        # hace que se ejecute el comando al iniciar el terminal 
	vt.fork_command("./blender")
